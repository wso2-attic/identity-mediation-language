...
import static org.wso2.carbon.ibus.mediation.cheetah.config.dsl.internal.flow.mediators.CustomMediatorBuilder.process;
...
public class SAMLtoOIDCDSL extends JavaConfigurationBuilder {
    ...
    public IntegrationFlow configure() {
        ...
        router.inboundEndpoint("inboundEndpoint1", http(port(8280), context("/sample/request"))).
                pipeline("pipeline1").
                filter(condition(source("Referer", Scope.Transport), pattern(".*samlsso.*"))).
